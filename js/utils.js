// js/utils.js
import { state, maskValue } from "./state.js";

export function encodeYahooTicker(ticker) {
  if (!ticker) return "";
...
export function safeValue(val, isName = false) {
...
export function formatToEokWon(val) {
  const num = parseSafeFloat(val);
  return (num / 100000000).toFixed(1) + "억원";
}

export function formatValueByMode(val, isKRW = true) {
  const num = parseSafeFloat(val);
  const isMobile =
    userViewMode === "mobile" ||
...
export function getResponsiveValueHTML(valStr) {
...
export function formatKRWInteger(val) {
...
export function formatKoreanCap(num) {
  if (num >= 1e12) {
