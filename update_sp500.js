...
async function run() {
    try {
        ...
        for (let i = 0; i < top100.length; i += batchSize) {
            ...
            console.log(`Processing batch ${i / batchSize + 1}/${top100.length / batchSize}...`);
            ...
            await Promise.all(batch.map(async (quote) => {
                ...
                const dividendYield = quote.trailingAnnualDividendYield ? (quote.trailingAnnualDividendYield * 100).toFixed(2) : "0.00";
        ...
        console.log(`Successfully saved ${results.length} stocks to sp500_data.json`);
    } catch (e) {
